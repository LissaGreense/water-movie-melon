// BUCKET STATICS
export const BUCKET_HEIGHT_VH = 0.75;
export const BUCKET_SCALE = 0.55;
export const INNER_BUCKET_RATIO = 0.6;
export const HEIGHT_TO_WIDTH_BUCKET_RATIO = 0.78;
export const VERTICAL_CANVAS_HEIGHT_SCALE = 0.88;
export const VERTICAL_CANVAS_WIDTH_SCALE = 0.8;
export const BOUNDARIES_OFFSET = 10;
export const GRAVITY_TIMESTAMP_SCALE = 0.006;
export const BOUNDARIES_SCALE = 0.03;

export const renderOptions = {
  strokeStyle: "transparent",
  fillStyle: "transparent",
};
