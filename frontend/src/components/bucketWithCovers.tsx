...
export function BucketWithCovers() {
  ...
  function coversRender(covers: string[]) {
    ...
    Composite.add(engine.current.world, [
      Bodies.rectangle(container_height/2, -offset, container_width + 2 * offset, 50.5, options),
      Bodies.rectangle(container_height/2, container_height + offset, container_width + 2 * offset, 50.5, options),
      Bodies.rectangle(container_width + offset, container_width/2, 50.5, container_height + 2 * offset, options),
      Bodies.rectangle(-offset, container_width/2, 50.5, container_height + 2 * offset, options)
    ]);

    for (const cover of covers) {
      console.log(cover);
      Composite.add(engine.current.world, Bodies.rectangle(Math.random() * 200, Math.random() * 200, 30, 45, {
        render: {
          sprite: {
            xScale: 0.1,
            yScale: 0.1,
    ...
    Events.on(engine.current, 'beforeUpdate', function() {
      engine.current.gravity.x = Math.cos(engine.current.timing.timestamp * 0.006);
      engine.current.gravity.y = Math.sin(engine.current.timing.timestamp * 0.006);
    });
