...
export const PasswordChangeDialog: FC<PasswordChangeDialogProps> = ({
    ...
    const handlePasswordChange = async () => {
        await postNewPassword(getUsername(), oldPassword, newPassword).then((r: any) => {
            if (r["result"] !== undefined && r["result"] == 'OK') {
                setVisible(false);
            }
        }).catch(error => {
                alert("Wystąpił błąd przy zmianie hasła: " + error["response"]["data"]["error"]);
            }
