...
export async function getMovieNights(): Promise<MovieNight[]> {
    try {
        const response = await axios.get<MovieNight[]>(backend_url + movie_nights_endpoint, getAuthHeadersConfig());
...
export async function postMovieNight(host: string | null, night_date: string, location: string): Promise<void> {
    ...
    try {
        await axios.post(backend_url + movie_nights_endpoint, data, getAuthHeadersConfig());
    } catch (error) {
        console.error(error);
    }
}
...
export async function joinMovieNight(night: MovieNight | undefined, user: string | null, accept_date: string) : Promise<void> {
    ...
    console.log(night);
    try {
        await axios.post(backend_url + attendees_endpoint, data, getAuthHeadersConfig());
    } catch (error){
        console.error(error);
    }
}
...
export async function getAttendees() : Promise<Attendees[]> {
    try {
        const response = await axios.get<Attendees[]>(backend_url + attendees_endpoint, getAuthHeadersConfig());
...
export async function getRandomMovie(): Promise<string> {
    try {
        const response = await axios.get(backend_url + rand_movie_endpoint, getAuthHeadersConfig())
...
export async function getMovieDate(): Promise<Date> {
    try {
        const response = await axios.get(backend_url + movie_date_endpoint, getAuthHeadersConfig())
...
export async function checkForNights(): Promise<boolean> {
    try {
        const response = await axios.get(backend_url + night_check_endpoint, getAuthHeadersConfig())
